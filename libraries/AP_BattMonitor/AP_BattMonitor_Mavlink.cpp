...
bool AP_BattMonitor_Mavlink::has_cell_voltages() const
{
    return HasCellVoltages;
}
...
void AP_BattMonitor_Mavlink::handle_mavlink(const mavlink_message_t &msg, uint8_t instance )
{
    ...
    int i=0;
    float ftemp=0;
    uint16_t cell_volts[14]={0};
    while(i<10 && !(status.voltages[i]==UINT16_MAX || status.voltages[i]==0))
    {
        ftemp += ((float)status.voltages[i])/1000;
        cell_volts[i]=status.voltages[i];
        i++;
        
        
    }
    if(i>1)
        HasCellVoltages=true;
        else
        HasCellVoltages=false;
    ...
    for(i=0;i<4;i++)
    {
        ftemp += ((float)status.voltages_ext[i])/1000;
        cell_volts[10+i]=status.voltages_ext[i];
    }
    if(HasCellVoltages==true)
        {
            for(int j=0;j<14;j++)
                _state.cell_voltages.cells[j]=cell_volts[j];
        }
    //mutex on these????
    WITH_SEMAPHORE(_msem);
    uint64_t tNow=AP_HAL::micros();
